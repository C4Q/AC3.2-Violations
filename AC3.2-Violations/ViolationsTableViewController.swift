...
class ViolationsTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "violationCellIdentifier", for: indexPath) as! ViolationTableViewCell
        
        let violation = violations[indexPath.row]
        cell.titleLabel.text = "\(violation.dba)"
        cell.subtitleLabel.text = "Location: \(violation.building) \(violation.street), \(violation.boro), \(violation.zipcode)"
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let tappedCell = sender as? ViolationTableViewCell {
            if segue.identifier == "detailSegue" {
                let dvc = segue.destination as! DetailViewController
                let cellIndexPath = self.tableView.indexPath(for: tappedCell)!
                let selectedCell = violations[cellIndexPath.row]
                dvc.violatingRestaurant = selectedCell
            }
        }
